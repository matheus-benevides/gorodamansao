...
const Catalog = () => {
  ...
  const getProductTranslation = (p) => {
    ...
    const nameKey = `p_${key}_name`;
    const descKey = `p_${key}_desc`;
    const translatedName = t(nameKey);
    const translatedDesc = t(descKey);

    return {
      name: translatedName === nameKey ? p.name : translatedName,
      desc: translatedDesc === descKey ? p.description : translatedDesc
    };
  };
  ...
  return (
    <main className="pt-24 md:pt-40 pb-section-gap px-margin max-w-container-max mx-auto">
      ...
      >
        {products.map((p, i) => {
          const { name, desc } = getProductTranslation(p);
          return (
            <motion.div 
              key={i}
            ...
            >
              ...
              <h3 className="font-headline text-3xl mb-2 relative z-10">{name}</h3>
              <p className="font-body text-sm text-on-surface-variant mb-8 flex-grow relative z-10 opacity-80">{desc}</p>
